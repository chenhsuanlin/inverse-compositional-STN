...
def compose(pBatch,dpBatch,opt,name=None):
	with tf.name_scope("compose"):
		pMtrxBatch = vec2mtrxBatch(pBatch,opt)
		dpMtrxBatch = vec2mtrxBatch(dpBatch,opt)
		pMtrxNewBatch = tf.matmul(dpMtrxBatch,pMtrxBatch)
		pMtrxNewBatch = pMtrxNewBatch/pMtrxNewBatch[:,2:3,2:3]
		pNewBatch = mtrx2vecBatch(pMtrxNewBatch,opt)
